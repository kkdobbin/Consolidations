...
#Make shapefiles to export for Justin of points and PWS boundaries just the ones in the analysis
library(rgdal)
library(maptools)
Shapefile <- Cases[,3:4]
Shapefile <- left_join(Shapefile, Consolidated_Points, by = c("SystemID" = "PWSID"))
Shapefile <- st_as_sf(Shapefile)
writeOGR(Shapefile, dsn = "Outputs/Consolidatedpoints", layer = "map",
         driver = "ESRI Shapefile")
dir.create("tempdir")
writeOGR(obj=Shapefile, dsn="tempdir", layer="Shapefile", driver="ESRI Shapefile")
writeOGR(Shapefile, dsn = "consoldiatedpoints.gpx", layer = "consolidatedpoints", driver = "GPX", 
         dataset_options = "GPX_USE_EXTENSIONS=yes")

as(Recieving_boundaries, "Spatial")
writeOGR(Recieving_boundaries, dsn = "Outputs/PWS_subset_receiving", layer = "map",
         driver = "ESRI Shapefile")
...
#Try with centroids
Recieving_centroids <- st_centroid(Recieving_boundaries_all)
st_crs(Recieving_centroids) 
Distance2 <- st_distance(Consolidated_points_all,  Recieving_centroids, by_element = TRUE)
Distance2 <- as.data.frame(Distance2)

# Old

library(geosphere)
Test <- dist2Line(Consolidated_points_all, Recieving_boundaries_all, distfun = distGeo)
#doesn't work

plot(st_geometry(Recieving_boundaries))
plot(st_geometry(Consolidated_Points))
plot(st_geometry(Recieving_centroids))

library(rgeos)
Test <- gCentroid(Recieving_boundaries_all, byid = TRUE)
